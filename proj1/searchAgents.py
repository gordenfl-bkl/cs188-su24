    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            for i in range(len(self.corners)):
                result.append(self.corners[i] == (nextx, nexty) or state[i + 1])

            next_state = ((nextx, nexty), result[0], result[1], result[2], result[3])
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i in range(len(corners)):
        if state[i + 1] is True:
            continue
        heuristic_val = max(
            heuristic_val,
            abs(state[0][0] - corners[i][0]) + abs(state[0][1] - corners[i][1]),
        )
    return heuristic_val  # Default to trivial solution
